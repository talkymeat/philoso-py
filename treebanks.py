...
class Treebank:
    ...
    Attributes:
        class_ids (dict): `dict` mapping from each label name to the unique
    ...
    def get_label(self, class_id = None):
        ...
        elif class_id in self.class_ids: ##- RAW, OK
            # ...get that label from the dict and return it...
            return self.class_ids[class_id] ##- RAW, OK
        else:
    ...
    def add_label(self, label):
        ...
        """
        if label.class_id in self.class_ids: ### BOTH, RAW
            raise AttributeError(
                f"The Treebank already contains a label {label!s}"
            )
        else:
            self.class_ids[label.class_id] = label  ### BOTH, RAW
    ...
    def print_all_labels(self, is_LaTeX=False):
        ...
        spacer = "======================"
        print(spacer)
        # Label.class_ids is a dict, so the loop can get the names *and* Labels
        for name, label in self.class_ids.items():
            print(f"{name}:")
    ...
    def get_all_root_nodes(self):
        ...
        """
        return {name: val.roots for name, val in self.class_ids.items()}
    ...
    def tree(self, treestr: str, **kwargs):
        ...
        if leaf:
            ...
            # the leaf string is taken as a literal, unless it corresponds to a
            # kwarg of the method call, in which case the kwarg is used: this is
            # how pandas arrays are used to make variables
            return self.T(self, the_label, (leaf if not leaf in kwargs else kwargs[leaf]), operator=operator)
...
class TypeLabelledTreebank(Treebank):
    ...
    def get_label(self, classtype):
        if isinstance(classtype, str):
            ...
            ct = classtype
        if ct in self.class_ids:
            # ...get that label from the dict and return it...
            return self.class_ids[ct]
        else:
            #...or, if it doesn't, then create it. The Label.__init__ method
            # will ensure the new Label goes in the dict.
    ...
    def make_class_name(self):
        """The superclass has this, but since class_ids in TypeLabelledTreebank
