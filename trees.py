...
from tree_errors import TreeIndexError, OperatorError
...
class NonTerminal(Tree):
    ...
    def __init__(self, treebank, label, *children, operator=None):
    ...
    def __str__(self):
        ...
        """
        return f"({self.label if self.label else ''}{self._operator}{''.join([str(child) for child in self]) if self else ''})"
...
class Label:
    ...
    Attributes:
        ...
        classname (str or type): Unique name for the `Label`.
    ...
    def __init__(self, treebank, classname = None):
    ...
    def to_LaTeX(self, op_name = ""):
        ...
        if self.is_default and not op_name:
            ...
            return f".{self.classname}:{op_name}"
        elif op_name:
            f".{op_name}"
        else:
...
class TypeLabel(Label):
    ...
    @property
    def classname(self, string = True):
        """(type or str) The name of the Label, and of the corresponding category of
        trees.
        """
        return self._classname.__name__ if string else self._classname
    ...
    def classname(self, name):
        ...
        if type(name) is not type: # Type check
            name = type(name)
        # OK, fine I guess.
        self._classname = name
    ...
    def __str__(self):
        ...
        >>> # implicitly also test this
        """
        return f"[{self._classname.__name__}]"

    def __repr__(self):
        return(f"TypeLabel('{self._classname.__name__}')")
    ...
    def to_LaTeX(self, op_name = ""):
        ...
        >>> # implicitly also test this
        """
        return ".{$" + op_name + (r'\rightarrow ' if op_name else '') + self._classname.__name__ + "$}"
...
class Treebank:
    ...
    def __init__(self, default_op = None, operators = None):
    ...
    def add_label(self, label):
        ...
        """
        if label._classname in self.classnames:
            raise AttributeError(
                f"The Treebank already contains a label {label!s}"
            )
        else:
            self.classnames[label._classname] = label
    ...
    def make_class_name(self): #
        ...
        return f"L{len(self.classnames)}"
    ...
    def tree(self, treestr: str):
        ...
        ... where `$leaf` is the the terminal content of the node — for
        ...
        # and children
        return NonTerminal(self, the_label, *child_list, operator=operator)
...
class TypeLabelledTreebank(Treebank):
    ...
    >>> bool(prod(list(not isinstance(tree, Tree) or type(tree.label._classname) == type for tree in DepthFirstBottomUp(gpt))))
    True
    >>> list(subtree() for subtree in filter(lambda x: isinstance(x, Tree), DepthFirstBottomUp(gpt)))
    ...
    def get_label(self, classtype):
        if type(classtype) == str:
            ...
            if type(ct) != type:
                raise AttributeError(
                    f"TypeLabel name {classname} does not evaluate to a type"
                )
        elif type(classtype) == type:
            ct = classtype
        if ct in self.classnames:
            # ...get that label from the dict and return it...
            return self.classnames[ct]
        else:
            #...or, if it doesn't, then create it. The Label.__init__ method
            # will ensure the new Label goes in the dict.
            return TypeLabel(self, ct)
    ...
    def make_class_name(self):
        """The superclass has this, but since classnames in TypeLabelledTreebank
    ...
    def verify_terminal(self, term):
        if term.label._classname == type(term[0]):
            return True
        else:
            raise OperatorError(
                f"{term} has a mismatch of label-type " +
                f"({term.label._classname.__name__}) and leaf-type " +
                f"({type(term[0]).__name__})"
            )

    def verify_nonterminal(self, nonterm):
        if nonterm.label._classname != nonterm._operator.return_type:
            raise OperatorError(
                f"{nonterm} has a mismatch of label-type " +
                f"({nonterm.label._classname.__name__}) and operator " +
                f"return-type ({nonterm._operator.return_type.__name__})"
            )
        if not nonterm._operator._type_seq_legal(*[ch.label._classname for ch in nonterm]):
            raise OperatorError(
...
def tree(treestr: str, treebank = None):
    if not treebank:
        treebank = Treebank()
