...
class Operator:
    ...
    def __init__(self, func, name, return_type = Any, arg_regex = "", force_type = False, force_type_lossy = False):
        self.func = func
        self.name = name
    ...
    def __call__(self, *args):
        ...
        def custom_type_err(output):
            ...
            # needed on thre branches of the tree, using an inner function
        ...
        if self._arg_seq_legal(*args):
            ...
            if self.return_type is Any or isinstance(output, self.return_type):
                return output
            # ... If it isn't ...
            else:
    ...
    def _type_ify(cls, arg):
        ...
        recorded in the `dtypes_dict`, to save computation next time
        """
        if type(arg) == pd.Series:
            dt = cls.dtype_dict.get(
                arg.dtype,
                cls.dtype_dict.get(type(arg.dtype), False)
            )
            if dt:
                return dt
            else:
                for k in cls.type_dict.keys():
                    if arg.dtype == k:
                        cls.dtype_dict[arg.dtype] = k
                        return k
                cls.dtype_dict[arg.dtype] = _DoesNothing
                return _DoesNothing
        return type(arg)
...
CONCAT = Operator(_concat, "CONCAT", str, r"s+")
"""
...
SUM = Operator(_sum, "SUM", float, r"[if]*", True)
"""
...
PROD = Operator(_prod, "PROD", float, r"[if]*", True)
"""
...
SQ = Operator(_sq, "SQ", float, r"[if]", True)
"""
...
CUBE = Operator(_cube, "CUBE", float, r"[if]", True)
"""
...
POW = Operator(_pow, "POW", float, r"[if][if]", True)
"""
...
GT = Operator(_gt, bool, r"[bif][bif]")
"""
...
NOT = Operator(_not, "NOT", bool, r"b")
"""
...
a devil's advocate and friend of free and open discussion. Actually just an
asshole.
...
TERN = Operator(_tern, "TERN", Any, r"b..")
"""
